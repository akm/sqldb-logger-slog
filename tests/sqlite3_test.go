...
func TestWithSqlite3(t *testing.T) {
	...
	t.Run("CREATE TABLE", func(t *testing.T) {
		...
		_, err := pool.Exec("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, age INTEGER)")
		assert.NoError(t, err)
		t.Logf("log: %s\n", buf.String())
		...
		assert.Equal(t, "CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, age INTEGER)", entry.Query)
